...
final class SinoIMEInstaller {
  enum InputMode: String, CaseIterable {
    static let primary = Self.hans
    case hans = "org.hannom.inputmethod.SinoIME.Hans"
    case hant = "org.hannom.inputmethod.SinoIME.Hant"
  }
  ...
  func register() {
    ...
    if !enabledInputModes.isEmpty {
      print("User already registered SinoIME method(s): \(enabledInputModes.map { $0.rawValue })")
      return
    }
    TISRegisterInputSource(SinoIMEApp.appDir as CFURL)
    print("Registered input source from \(SinoIMEApp.appDir)")
  }

  func enable(modes: [InputMode] = []) {
    let enabledInputModes = enabledModes()
    if !enabledInputModes.isEmpty && modes.isEmpty {
      print("User already enabled SinoIME method(s): \(enabledInputModes.map { $0.rawValue })")
