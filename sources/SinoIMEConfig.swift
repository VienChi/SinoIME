...
final class SinoIMEConfig {
  private let rimeAPI: RimeApi_stdbool = rime_get_api_stdbool().pointee
  private(set) var isOpen = false

  private var cache: [String: Any] = [:]
  private var config: RimeConfig = .init()
  private var baseConfig: SinoIMEConfig?

  func openBaseConfig() -> Bool {
    close()
    isOpen = rimeAPI.config_open("sinoime", &config)
    return isOpen
  }

  func open(schemaID: String, baseConfig: SinoIMEConfig?) -> Bool {
    close()
  ...
  func getColor(_ option: String, inSpace colorSpace: SinoIMETheme.RimeColorSpace) -> NSColor? {
...
private extension SinoIMEConfig {
  func cachedValue<T>(of: T.Type, forKey key: String) -> T? {
    return cache[key] as? T
  }

  func color(from colorStr: String, inSpace colorSpace: SinoIMETheme.RimeColorSpace) -> NSColor? {
  ...
  func color(alpha: Int, red: Int, green: Int, blue: Int, colorSpace: SinoIMETheme.RimeColorSpace) -> NSColor {
    switch colorSpace {
    case .displayP3:
