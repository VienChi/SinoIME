...
final class SinoIMEPanel: NSPanel {
  private let view: SinoIMEView
  private let back: NSView
  var inputController: SinoIMEInputController?
  ...
  init(position: NSRect) {
    ...
    self.view = SinoIMEView(frame: position)
  ...
  func load(config: SinoIMEConfig, forDarkMode isDark: Bool) {
    if isDark {
      view.darkTheme = SinoIMETheme()
      view.darkTheme.load(config: config, dark: true)
    } else {
      view.lightTheme = SinoIMETheme()
      view.lightTheme.load(config: config, dark: isDark)
    }
  }
}

private extension SinoIMEPanel {
    ...
    if requiresFullScreen {
      ...
      if vertical {
        ...
        if position.midY / screenRect.height >= 0.5 {
          panelRect.origin.y = position.minY - SinoIMETheme.offsetHeight - panelRect.height + theme.pagingOffset
        } else {
          panelRect.origin.y = position.maxY + SinoIMETheme.offsetHeight
        }
        panelRect.origin.x = position.minX - panelRect.width - SinoIMETheme.offsetHeight
        ...
        panelRect.origin = NSPoint(x: position.minX - theme.pagingOffset, y: position.minY - SinoIMETheme.offsetHeight - panelRect.height)
      }
      ...
      if panelRect.minY < screenRect.minY {
        if vertical { panelRect.origin.y = screenRect.minY } else { panelRect.origin.y = position.maxY + SinoIMETheme.offsetHeight }
      }
  ...
  func show(status message: String) {
    ...
    statusTimer?.invalidate()
    statusTimer = Timer.scheduledTimer(withTimeInterval: SinoIMETheme.showStatusDuration, repeats: false) { _ in
      self.hide()
    }
  }
