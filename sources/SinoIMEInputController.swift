...
final class SinoIMEInputController: IMKInputController {
  ...
  private var chordKeyCodes: [UInt32] = .init(repeating: 0, count: SinoIMEInputController.keyRollOver)
  private var chordModifiers: [UInt32] = .init(repeating: 0, count: SinoIMEInputController.keyRollOver)
  ...
  override func handle(_ event: NSEvent!, client sender: Any!) -> Bool {
    ...
    case .flagsChanged:
      ...
      var rimeModifiers: UInt32 = SinoIMEKeycode.osxModifiersToRime(modifiers: modifiers)
      // Some remote desktop tools send flagsChanged with keyCode 0; infer the real modifier key when needed.
      var keyCode = event.keyCode
      if !SinoIMEKeycode.modifierKeycodes.contains(keyCode) {
        guard let inferred = SinoIMEKeycode.inferModifierKeycode(from: changes) else {
      ...
      let rimeKeycode: UInt32 = SinoIMEKeycode.osxKeycodeToRime(keycode: keyCode, keychar: nil, shift: false, caps: false)
    ...
    case .keyDown:
      ...
      if let char = keyChars?.first {
        let rimeKeycode = SinoIMEKeycode.osxKeycodeToRime(keycode: keyCode, keychar: char,
                                                           shift: modifiers.contains(.shift),
                                                           caps: modifiers.contains(.capsLock))
        if rimeKeycode != 0 {
          let rimeModifiers = SinoIMEKeycode.osxModifiersToRime(modifiers: modifiers)
  ...
  override func activateServer(_ sender: Any!) {
    self.client ?= sender as? IMKTextInput
    var keyboardLayout = NSApp.sinoimeAppDelegate.config?.getString("keyboard_layout") ?? ""
    ...
    if session != 0 {
      ...
      NSApp.sinoimeAppDelegate.updateStatusIcon(asciiMode: state, schemaLabel: label)
    }
  }
  ...
  override init!(server: IMKServer!, delegate: Any!, client: Any!) {
    ...
    NotificationCenter.default.addObserver(
      forName: .init("SinoIMESetASCIIModeNotification"),
      object: nil,
      queue: nil
    ...
    NotificationCenter.default.addObserver(
      forName: .init("SinoIMEReportASCIIModeNotification"),
      object: nil,
      queue: nil
    ) { [weak self] notification in
  ...
  override func hidePalettes() {
    NSApp.sinoimeAppDelegate.panel?.hide()
    super.hidePalettes()
  }
  ...
  @objc func deploy() {
    NSApp.sinoimeAppDelegate.deploy()
  }

  @objc func syncUserData() {
    NSApp.sinoimeAppDelegate.syncUserData()
  }

  @objc func openLogFolder() {
    NSApp.sinoimeAppDelegate.openLogFolder()
  }

  @objc func openRimeFolder() {
    NSApp.sinoimeAppDelegate.openRimeFolder()
  }

  @objc func checkForUpdates() {
    NSApp.sinoimeAppDelegate.checkForUpdates()
  }

  @objc func openWiki() {
    NSApp.sinoimeAppDelegate.openWiki()
  }
...
private extension SinoIMEInputController {
  ...
  func updateChord(keycode: UInt32, modifiers: UInt32) {
    ...
    chordDuration = 0.1
    if let duration = NSApp.sinoimeAppDelegate.config?.getDouble("chord_duration"), duration > 0 {
  ...
  func createSession() {
    let app = client?.bundleIdentifier() ?? {
      SinoIMEInputController.unknownAppCnt &+= 1
      return "UnknownApp\(SinoIMEInputController.unknownAppCnt)"
    }()
  ...
  func updateAppOptions() {
    ...
    if let appOptions = NSApp.sinoimeAppDelegate.config?.getAppOptions(currentApp) {
      for (key, value) in appOptions {
        print("set app option: \(key) = \(value)")
        rimeAPI.set_option(session, key, value)
      }
    }
    if let reportBundleID = NSApp.sinoimeAppDelegate.config?.getBool("unsafe/report_bundleid"), reportBundleID {
  ...
  func processKey(_ rimeKeycode: UInt32, modifiers rimeModifiers: UInt32) -> Bool {
    if let panel = NSApp.sinoimeAppDelegate.panel {
  ...
  func rimeUpdate(clearReservedComments: Bool = true) {
    ...
    if rimeAPI.get_status(session, &status) {
      ...
      if let schema_id = status.schema_id, schemaId == "" || schemaId != String(cString: schema_id) {
        ...
        NSApp.sinoimeAppDelegate.loadSettings(for: schemaId)
        if let panel = NSApp.sinoimeAppDelegate.panel {
