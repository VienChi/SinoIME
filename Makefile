...
release: $(DEPS_CHECK)
	...
	xcodebuild -project SinoIME.xcodeproj -configuration Release -scheme SinoIME -derivedDataPath $(DERIVED_DATA_PATH) $(BUILD_SETTINGS) build

debug: $(DEPS_CHECK)
	mkdir -p $(DERIVED_DATA_PATH)
	bash package/add_data_files
	xcodebuild -project SinoIME.xcodeproj -configuration Debug -scheme SinoIME -derivedDataPath $(DERIVED_DATA_PATH)  $(BUILD_SETTINGS) build
...
ifdef DEV_ID
	productsign --sign "Developer ID Installer: $(DEV_ID)" package/SinoIME.pkg package/SinoIME-signed.pkg
	rm package/SinoIME.pkg
	mv package/SinoIME-signed.pkg package/SinoIME.pkg
	xcrun notarytool submit package/SinoIME.pkg --keychain-profile "$(DEV_ID)" --wait
	xcrun stapler staple package/SinoIME.pkg
endif
...
permission-check:
	[ -w "$(DSTROOT)" ] && [ -w "$(SINOIME_APP_ROOT)" ] || sudo chown -R ${USER} "$(DSTROOT)"

install-debug: debug permission-check
	rm -rf "$(SINOIME_APP_ROOT)"
	cp -R $(DERIVED_DATA_PATH)/Build/Products/Debug/SinoIME.app "$(DSTROOT)"
	DSTROOT="$(DSTROOT)" RIME_NO_PREBUILD=1 bash scripts/postinstall

install-release: release permission-check
	rm -rf "$(SINOIME_APP_ROOT)"
	cp -R $(DERIVED_DATA_PATH)/Build/Products/Release/SinoIME.app "$(DSTROOT)"
